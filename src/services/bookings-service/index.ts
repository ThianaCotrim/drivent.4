...
import bookingRepository from 'repositories/bookings-repository/index';
...
async function postBooking(userId: number, roomId: number) {
  ...
  const room = await bookingRepository.findBookingObj(booking.roomId);
  ...
  if (room.id === 0) throw genericError('RoomError', 'Room is already full');
...
async function putBooking(userId: number, roomId: number, bookingId: number) {
  const booking = await bookingRepository.findBookingByuserId(userId);
  const newRoom = await bookingRepository.findBookingObj(roomId);
  if (!booking) throw { message: "This user doesn't have a booking to chage", type: 'BookingError' };
  if (newRoom.id === 0) throw { message: 'Room is already full', type: 'RoomError' };
...
const bookingService = { getBookings, postBooking, putBooking };
